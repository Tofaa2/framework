...
fn resolvePosition(world: *root.World, entity: anytype, transform: *const root.Transform, renderer: *root.Renderer) [2]f32 {
...
fn updateLights(world: *root.World, renderer: *root.Renderer, app: *root.App) void {
    ...
    var light_iter = world.view(&.{ root.Transform, root.Light }, .{});
    while (light_iter.next()) |entity_t| {
        const entity = entity_t.entity;
        if (light_count >= MAX_LIGHTS) break;
        const transform = world.getConst(root.Transform, entity);
        const light = world.getConst(root.Light, entity);
...
fn render2D(world: *root.World, builder: *root.MeshBuilder, app: *root.App) void {
    ...
    var iter = world.view(&.{ root.Transform, root.Renderable }, .{});

    while (iter.next()) |entity_t| {
        const entity = entity_t.entity;
        const transform = world.getConst(root.Transform, entity);
        const renderable = world.getConst(root.Renderable, entity);
...
fn renderSystem(world: *root.World) anyerror!void {
