...
fn plugin_build(app: *root.App) void {
    ...
    app.world.scheduler.add(&.{CollisionEvents}, clearCollisionEvents);
    app.world.scheduler.add(&.{ root.Gravity, root.Velocity, root.RigidBody }, applyGravity);
    app.world.scheduler.add(&.{ root.RigidBody, root.Transform, root.Velocity }, integrateVelocity);
    app.world.scheduler.add(&.{ root.Collider, root.RigidBody }, detectAndResolveCollisions);
...
fn applyGravity(world: *root.World) !void {
    ...
    var iter = world.view(&.{ root.Gravity, root.Velocity }, .{});
    while (iter.next()) |entity_t| {
        const entity = entity_t.entity;
        if (world.tryGetConst(root.RigidBody, entity)) |body| {
            if (body.is_static) continue;
        }
        const gravity = world.getConst(root.Gravity, entity);
        var vel = world.get(root.Velocity, entity);
...
fn integrateVelocity(world: *root.World) !void {
    ...
    var drag_iter = world.view(&.{ root.RigidBody, root.Velocity }, .{});
    while (drag_iter.next()) |entity_t| {
        const entity = entity_t.entity;
        const body = world.getConst(root.RigidBody, entity);
        if (body.is_static) continue;
        var vel = world.get(root.Velocity, entity);
    ...
    var iter = world.view(&.{ root.Transform, root.Velocity }, .{});
    while (iter.next()) |entity_t| {
        const entity = entity_t.entity;
        if (world.tryGetConst(root.RigidBody, entity)) |body| {
            if (body.is_static) continue;
        ...
        const transform = world.get(root.Transform, entity);
        const vel = world.getConst(root.Velocity, entity);
...
fn detectAndResolveCollisions(world: *root.World) void {
    ...
    var iter = world.view(&.{ root.Transform, root.Collider }, .{});
    while (iter.next()) |entity_t| {
        const entity = entity_t.entity;
        const transform = world.getConst(root.Transform, entity);
        const collider = world.getConst(root.Collider, entity);
