...
pub const ObjMaterial = struct {
    diffuse: Color = .white,
    diffuse_texture_path: ?[]u8 = null,

    pub fn deinit(self: *ObjMaterial, allocator: std.mem.Allocator) void {
        if (self.diffuse_texture_path) |p| allocator.free(p);
    }
};
...
pub const Obj = struct {
    ...
    pub fn deinit(self: *Obj) void {
        ...
        self.material.deinit(self.allocator);
    }
};

fn parseMtl(allocator: std.mem.Allocator, path: []const u8) !std.StringHashMap(MtlMaterial) {
    var map = std.StringHashMap(MtlMaterial).init(allocator);
...
pub fn load(allocator: std.mem.Allocator, path: []const u8) !Obj {
    ...
    var materials = std.StringHashMap(MtlMaterial).init(allocator);
    defer {
    ...
    var current_texture_path: ?[]u8 = null;
        ...
        if (std.mem.startsWith(u8, trimmed, "mtllib ")) {
            ...
            if (materials.get(mat_name)) |mat| {
                ...
                if (mat.diffuse_texture) |tex_name| {
                    // free previous path if switching materials
                    if (current_texture_path) |p| allocator.free(p);
                    const full_path = try std.fs.path.join(allocator, &.{ dir, tex_name });
                    current_texture_path = full_path;
                } else {
                    if (current_texture_path) |p| allocator.free(p);
                    current_texture_path = null;
                }
            }
    ...
    return Obj{
        .allocator = allocator,
        .vertices = try allocator.dupe(Vertex, builder.vertices.items),
        .indices = try allocator.dupe(u16, builder.indices.items),
        .material = .{
            .diffuse = current_color,
            .diffuse_texture_path = current_texture_path,
        },
    };
}
