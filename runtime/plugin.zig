        ...
        pub fn add(self: *Self, plugin: anytype) PluginError!void {
            ...
            if (@hasDecl(plugin_type, "dependencies")) {
                inline for (plugin_type.dependencies) |DepType| {
                    deps.append(self.allocator, utils.TypeInfo.get(DepType).id) catch return error.AllocationFailed;
                }
            }
            ...
            std.debug.print("TypeID: {d}\n", .{utils.TypeInfo.get(plugin_type).id});
            const erased = TypeErased{
                .ptr = heaped,
                .type_id = utils.TypeInfo.get(plugin_type),
